...
## 테이블
### 테이블명: user
유저 정보를 담는 유저 마스터 테이블

| 컬럼명        | 데이터 타입 | 설명                      |
|---------------|-------------|-------------------------|
| id            | bigint      | 유저 고유 식별자 (Primary Key) |
| name          | varchar     | 유저 이름                   |
| created_at    | date        | 데이터 생성일                 |
| updated_at    | date        | 데이터 수정일                 |

### 테이블명: point
유저의 포인트 정보를 담는 테이블
| 컬럼명        | 데이터 타입 | 설명                            |
|---------------|-------------|---------------------------------|
| id            | bigint      | 포인트 고유 식별자 (Primary Key)   |
| user_id       | bigint      | 유저 번호                     |
| point         | decimal     | 유저가 보유한 포인트             |
| created_at    | date        | 데이터 생성일                   |
| updated_at    | date        | 데이터 수정일                   |

### 테이블명: point_history
포인트 충전, 사용 이력을 쌓는 테이블

| 컬럼명                | 데이터 타입 | 설명                           |
|--------------------|-------------|------------------------------|
| id                 | bigint      | 포인트 이력 고유 식별자 (Primary Key)  |
| point_id           | bigint      | 포인트 ID                       |
| point_history_type | varchar     | 포인트 발생 유형 (CHARGE-충전/USE-사용) |
| amount             | decimal     | 금액                           |
| created_at         | datetime    | 포인트 이력 생성일                   |

### 테이블명: coupon
쿠폰 정보를 담고 있는 쿠폰 마스터 테이블
...
| discount_amt     | decimal  | 할인 금액                                   |
| max_capacity     | int      | 최대 발급 허용 개수                             |
| remain_capacity  | int      | 잔여 쿠폰 개수                                |
| valid_started_at | datetime | 쿠폰 사용 기간 시작일                            |
| valid_ended_at   | datetime | 쿠폰 사용 기간 종료일                            |
| status           | varchar  | 쿠폰 상태(ACTIVE-발급 및 사용 가능/DEACTIVATED-관리자 비활성화) |
| created_at       | datetime | 데이터 생성일                                 |
| updated_at       | datetime | 데이터 수정일                                 |

### 테이블명: issued_coupon
유저가 보유하고 있는 쿠폰 정보를 담는 테이블
...
| total_original_amt | int         | 할인 전 총 금액(순수 구매 금액)            |
...
| updated_at    | datetime | 주문 상세 데이터 수정일              |
...
| total_original_amt | decimal  | 할인 전 총 금액(순수 구매 금액)                    |
| discount_amt       | decimal  | 할인 금액                                  |
| final_payment_amt  | decimal  | 최종 결제 금액                               |
| issued_coupon_id   | bigint   | 사용한 할인 쿠폰 ID                           |
| created_at         | datetime | 결제 데이터 생성일                             |
| updated_at         | datetime | 결제 데이터 수정일                             |

### 테이블명: order_created_outbox 
주문 완료 이벤트 아웃박스 테이블 

| 컬럼명        | 데이터 타입   | 설명                                |
|------------|----------|-----------------------------------|
| message_id | bigint   | 메세지 키 (Primary Key)               |
| order_id   | bigint   | 주문 ID                             |
| payload    | varchar  | 주문 이벤트 상세 정보 (JSON 형식)            |
| status     | decimal  | 진행 상태(INIT-메세지 저장/COMPLETE-발송 성공) |
| created_at | datetime | 데이터 생성일                           |
---- 
