...
public class PointController {
    ...
    public ApiResponse<PointResponse> getUserPoint(@PathVariable Long userId) {
        return ApiResponse.ok(PointResponse.of(pointService.getPoint(userId)));
    }

    @Operation(summary = "잔액 충전 API", description = "유저의 보유 잔액을 충전하고 반환한다.")
    @PostMapping("/charge")
    public ApiResponse<PointResponse> chargeUserPoint(@RequestBody PointChargeRequest request) {
        return ApiResponse.ok(PointResponse.of(pointService.charge(request.userId(), request.amount())));
    }
}
